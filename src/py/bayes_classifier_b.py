...
#		assuming the case when all classes have full but equal Covariance Matrices.
	...
	for i in range(len (classes)):
		...
		for j in range(dimension):
			for k in range(dimension):
				tempFirstTerm[0,j]+=(x[j]-mean[i][j])*covarianceMatrixInv[j,k]
		for j in range(dimension):
			first_term+=tempFirstTerm[0,j]*(x[j]-mean[i][j])
		val[i]*=first_term
		tot=0
...
def g(x,first,second):
	...
	for j in range(dimension):
		for k in range(dimension):
			tempFirstTerm[0,j]+=(x[j]-mean[first][j])*covarianceMatrixInv[j,k]
	for j in range(dimension):
		first_term+=tempFirstTerm[0,j]*(x[j]-mean[first][j])
	second_term=0
	tempSecondTerm=np.zeros(shape=(1,dimension))
	for j in range(dimension):
		for k in range(dimension):
			tempSecondTerm[0,j]+=(x[j]-mean[second][j])*covarianceMatrixInv[j,k]
	for j in range(dimension):
		second_term+=tempSecondTerm[0,j]*(x[j]-mean[second][j])
...
print "\nThe average Covariance Matrix calculated for all classes comes out to be"
print covarianceMatrix

print "\nThe mean and variance vectors for different classes are: \n"

for i in range(len(classes)):
	print "Class ",i+1,": Mean - ",mean[i]," Var - [",covarianceMatrices[i][0,0]," ",covarianceMatrices[i][1][1],"]"
